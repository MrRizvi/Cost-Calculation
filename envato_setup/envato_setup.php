...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		/** @var string Full url to server-script.php (available from https://gist.github.com/webinane ) */
		...
		public function get_logo_image() {
			...
			if ( $logo_image_id ) {
				...
				$image_url = get_theme_mod( 'logo_header_image', get_template_directory_uri() . '/partial/images/logo.png' );
			}
		...
		public function init_globals() {
			...
			$this->envato_username = apply_filters( $this->theme_name . '_theme_setup_wizard_username', 'webinane' );
			$this->oauth_script    = apply_filters( $this->theme_name . '_theme_setup_wizard_oauth_script', 'http://api.webinane.com/envato/api/server-script.php' );
...
//                'style2' => 'Style 2',
		...
		public function init_actions() {
			if ( apply_filters( $this->theme_name . '_enable_setup_wizard', true ) && current_user_can( 'manage_options' ) ) {
				...
				add_action( 'wp_ajax_envato_setup_media_content_download', array( $this, 'remaining_media' ) );
			}
		...
		public function admin_menus() {
			...
			if ( $this->is_submenu_page() ) {
				...
				$add_subpage_function( $this->parent_slug, esc_html__( 'Setup Wizard','unload' ), esc_html__( 'Setup Wizard' ,'unload'), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			} else {
				add_theme_page( esc_html__( 'Setup Wizard' ,'unload'), esc_html__( 'Setup Wizard','unload' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			}
		...
		public function init_wizard_steps() {
			...
			$this->steps = array(
				'introduction' => array(
					'name'    => esc_html__( 'Introduction','unload' ),
...
//			);
		...
		public function setup_wizard() {
			...
			wp_localize_script( 'envato-setup', 'envato_setup_params', array(
				...
				'verify_text'      => esc_html__( '...verifying','unload' ),
			) );
		...
		public function setup_wizard_header() {
			...
			// avoid theme check issues.
				echo '<t';
				echo 'itle>' . esc_html__( 'Theme &rsaquo; Setup Wizard','unload' ) . '</ti' . 'tle>'; ?>
			...
			<body class="envato-setup wp-core-ui">
				<h1 id="wc-logo">
					<a href="http://themeforest.net/user/webinane/portfolio" target="_blank"><?php
		...
		public function setup_wizard_footer() {
			...
			<?php if ( 'next_steps' === $this->step ) : ?>
				...
				href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard','unload' ); ?></a>
			<?php endif; ?>
		</body>
		<?php
		...
		public function envato_setup_introduction() {
			...
			if ( false && isset( $_REQUEST['debug'] ) ) {
				...
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.','unload' ), wp_get_theme() ); ?></h1>
				<p><?php esc_html_e( 'It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.','unload' ); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!' ,'unload'); ?></a>
						<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
							class="button button-large"><?php esc_html_e( 'Not right now','unload' ); ?></a>
						</p>
						<?php
					} else if ( get_option( 'envato_setup_complete', false ) ) {
						?>
						<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.','unload' ), wp_get_theme() ); ?></h1>
						<p><?php esc_html_e( 'It looks like you have already run the setup wizard. Below are some options: ','unload' ); ?></p>
						<ul>
							<li>
								<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
									class="button-primary button button-next button-large"><?php esc_html_e( 'Run Setup Wizard Again','unload' ); ?></a>
								</li>
								<li>
									<form method="post">
										<input type="hidden" name="reset-font-defaults" value="yes">
										<input type="submit" class="button-primary button button-large button-next"
										value="<?php esc_attr_e( 'Reset font style and colors','unload' ); ?>" name="save_step"/>
							...
							<p class="envato-setup-actions step">
								<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
									class="button button-large"><?php esc_html_e( 'Cancel' ,'unload'); ?></a>
								</p>
								<?php
							} else {
								?>
								<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.','unload' ), wp_get_theme() ); ?></h1>
								<p><?php printf( esc_html__( 'Thank you for choosing the %s theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. It should only take 5 minutes.','unload' ), wp_get_theme() ); ?></p>
								<p><?php esc_html_e( 'No time right now? If you don\'t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!','unload' ); ?></p>
								<p class="envato-setup-actions step">
									<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
										class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!' ,'unload'); ?></a>
										<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
		...
		public function envato_setup_introduction_save() {
			...
			if ( ! empty( $_POST['reset-font-defaults'] ) && $_POST['reset-font-defaults'] == 'yes' ) {
				...
				?>
				<p>
					<strong><?php esc_html_e( 'Options have been reset. Please go to Appearance > Customize in the WordPress backend.','unload' ); ?></strong>
				</p>
				<?php
				return true;
			}
		 ...
		 * Page setup
		 */
		public function envato_setup_download_media() {

			?>
			<h1><?php esc_html_e( 'Default Content' ,'unload'); ?></h1>
			<form method="post">
				<?php if ( $this->is_possible_upgrade() ) { ?>
				<p><?php esc_html_e( 'It looks like you already have media installed on this website. If you would like to install the default demo media as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.','unload' ); ?></p>
				<?php } else { ?>
				<p><?php printf( esc_html__( 'It\'s time to download some media for your new WordPress website. Choose media if you would like to download and click Continue. It is recommended to leave it selected. Once inserted, this media can be managed from the WordPress admin dashboard. ','unload' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
				<table class="envato-setup-pages" cellspacing="0">
					<thead>
						<tr>
							<td class="check"></td>
							<th class="item"><?php esc_html_e( 'Item','unload' ); ?></th>
							<th class="description"><?php esc_html_e( 'Description','unload' ); ?></th>
							<th class="status"><?php esc_html_e( 'Status','unload' ); ?></th>
						</tr>
					</thead>
					<tbody>
						<tr class="envato_default_content" data-content="download_media">
							<td>
								<input type="checkbox" name="default_content[downoad_media]"
								class="envato_default_content"
								id="default_content_download_media"
								value="1" checked="checked">
							</td>
							<td><label
								for="default_content_download_media"><?php esc_html_e( 'Download Media', 'unload' ); ?></label>
							</td>
							<td class="description"><?php esc_html_e( 'This will download default Media as seen in the demo.', 'unload' ); ?></td>
							<td class="status"><span></span>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						...
						data-callback="download_media_data"><?php esc_html_e( 'Continue','unload' ); ?></a>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							class="button button-large button-next"><?php esc_html_e( 'Skip this step','unload' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
					</form>
					<?php
				...
				function ajax_media_data() {
					...
					if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
						wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No content Found','unload' ) ) );
					}
					...
					if ( true ) {
				// install the content!
						$upload_dir = unload_set( wp_get_upload_dir(), 'basedir' );
						if ( ! is_dir( $upload_dir . '/unload' ) ) {
							$wp_filesystem->mkdir($upload_dir . '/unload');
						}
						$zipfile = $upload_dir . '/unload/media.zip';

						$this->log( ' -!! STARTING SECTION for ' . $_POST['content'] );
						set_time_limit(0);

						if ( file_exists( $zipfile ) ) {
							unzip_file( $zipfile, $upload_dir . '/unload' );
						} else {
							$file = wp_remote_get( 'https://s3.amazonaws.com/webinane-downloads/unload_media.zip', array('timeout' => 300 ) );

							if ( is_wp_error( $file ) ) {
								printr($file);
							}
							if ( $body = wp_remote_retrieve_body( $file ) ) {

								if ( ! file_exists( $zipfile ) ) {

									$wp_filesystem->put_contents( $zipfile, $body );
						...
						$json = array(
							'done'    => 1,
							'message' => esc_html__( "Success", 'unload' ),
							'debug'   => $result,
							'logs'    => $this->logs,
							'errors'  => $this->errors,
						);
				...
				wp_send_json( array(
					'error'   => 1,
					'message' => esc_html__( 'Error' ,'unload'),
					'logs'    => $this->logs,
					'errors'  => $this->errors,
				) );
			}
		...
		public function envato_setup_default_plugins() {
			...
			?>
			<h1><?php esc_html_e( 'Default Plugins','unload' ); ?></h1>
			<form method="post">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed or updated:' ,'unload'); ?></p>
					...
					echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.','unload'  ) . '</strong></p>';
				} ?>

				<p><?php esc_html_e( 'You can add and remove plugins later on from within WordPress.','unload' ); ?></p>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						class="button-primary button button-large button-next"
						data-callback="install_plugins"><?php esc_html_e( 'Continue','unload' ); ?></a>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							class="button button-large button-next"><?php esc_html_e( 'Skip this step','unload' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
				...
				public function ajax_plugins() {
					if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
						wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found' ,'unload') ) );
					}
					$json = array();
			...
			// what are we doing with this plugin?
					foreach ( $plugins['activate'] as $slug => $plugin ) {
						if ( $_POST['slug'] == $slug ) {
							$json = array(
								...
								'action2'       => - 1,
								'message'       => esc_html__( 'Activating Plugin','unload' ),
							);
							break;
						}
					}
					foreach ( $plugins['update'] as $slug => $plugin ) {
						if ( $_POST['slug'] == $slug ) {
							$json = array(
								...
								'action2'       => - 1,
								'message'       => esc_html__( 'Updating Plugin','unload' ),
							);
							break;
						}
					}
					foreach ( $plugins['install'] as $slug => $plugin ) {
						if ( $_POST['slug'] == $slug ) {
							$json = array(
								...
								'action2'       => - 1,
								'message'       => esc_html__( 'Installing Plugin','unload' ),
							);
							break;
						}
					}
				...
				wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success','unload' ) ) );
			}
			exit;
		...
		private function _content_default_get() {
			...
			foreach ( $available_content as $post_type => $post_data ) {
				if ( count( $post_data ) ) {
					...
					$content[ $post_type ] = array(
						...
						'description'      => sprintf( esc_html__( 'This will create default %s as seen in the demo.','unload' ), $post_type_title ),
						'pending'          => esc_html__( 'Pending.' ,'unload'),
						'installing'       => esc_html__( 'Installing.' ,'unload'),
						'success'          => esc_html__( 'Success.' ,'unload'),
			...
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets','unload' ),
				'description'      => esc_html__( 'Insert default sidebar widgets as seen in the demo.' ,'unload'),
				'pending'          => esc_html__( 'Pending.','unload' ),
				'installing'       => esc_html__( 'Installing Default Widgets.','unload' ),
				'success'          => esc_html__( 'Success.' ,'unload'),
			...
			$content['settings'] = array(
				'title'            => esc_html__( 'Settings' ,'unload'),
				'description'      => esc_html__( 'Configure default settings.' ,'unload'),
				'pending'          => esc_html__( 'Pending.','unload' ),
				'installing'       => esc_html__( 'Installing Default Settings.','unload' ),
				'success'          => esc_html__( 'Success.','unload' ),
				'install_callback' => array( $this, '_content_install_settings'),
				'checked'          => $this->is_possible_upgrade() ? 0 : 1,
				// dont check if already have content installed.
			);
			$content['layerslider'] = array(
				'title'            => esc_html__( 'LayerSlider','unload' ),
				'description'      => esc_html__( 'Install layerslider demo content.','unload' ),
				'pending'          => esc_html__( 'Pending.','unload' ),
				'installing'       => esc_html__( 'Installing layerslider content.','unload' ),
				'success'          => esc_html__( 'Success.' ,'unload'),
				'install_callback' => array( $this, 'layerslider_import' ),
				'checked'          => $this->is_possible_upgrade() ? 0 : 1,
				// dont check if already have content installed.
			);
//            $content['revslider'] = array(
//                'title'            => esc_html__( 'Revslider','unload' ),
//                'description'      => esc_html__( 'Install revslider demo content.','unload' ),
//                'pending'          => esc_html__( 'Pending.','unload' ),
//                'installing'       => esc_html__( 'Installing revslider content.','unload' ),
//                'success'          => esc_html__( 'Success.' ,'unload'),
//                'install_callback' => array( $this, 'revslider_import' ),
//                'checked'          => $this->is_possible_upgrade() ? 0 : 1,
//                // dont check if already have content installed.
//            );
		...
		public function envato_setup_default_content() {
			...
			<form method="post">
				<?php if ( $this->is_possible_upgrade() ) { ?>
				<p><?php esc_html_e( 'It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.','unload' ); ?></p>
				<?php } else { ?>
				<p><?php printf( esc_html__( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ','unload' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
				<table class="envato-setup-pages" cellspacing="0">
					<thead>
						<tr>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						class="button-primary button button-large button-next"
						data-callback="install_content"><?php esc_html_e( 'Continue','unload' ); ?></a>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							class="button button-large button-next"><?php esc_html_e( 'Skip this step','unload' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
		...
		private function _fetch_remote_file( $url, $post ) {
			...
			if ( ! $upload || $upload['error'] ) {
				...
				if ( is_array( $response ) && ! empty( $response['body'] ) && $response['response']['code'] == '200' ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond','unload' ) );
				}
				...
				if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote file is incorrect size','unload' ) );
				}

				if ( 0 == $filesize ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded','unload' ) );
				}

				if ( ! empty( $max_size ) && $filesize > $max_size ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote file is too large, limit is %s' ,'unload'), size_format( $max_size ) ) );
				}
			}
		...
		public function _get_json( $file ) {
			...
			$theme_style = __DIR__ . '/content/style1/';
		...
		public function envato_setup_color_style() {
			...
			?>
			<h1><?php esc_html_e( 'Site Style','unload' ); ?></h1>
			<form method="post">
				<p><?php esc_html_e( 'Please choose your site style below.','unload' ); ?></p>
					...
					<ul>
						...
						foreach ( $this->site_styles as $style_name => $style_data ) {
							...
							<a href="#" data-style="<?php echo esc_attr( $style_name ); ?>"><img
								src="<?php echo esc_url(get_template_directory_uri() .'/envato_setup/images/'.$style_name.'/style.jpg');?>">
							</a>
							<h3><?php
							if($style_name=='style1') { echo esc_attr('Main'); }
//                                    if($style_name=='style2') { echo esc_attr('Charity'); }
							?></h3>
						</li>
						<?php } ?>
					</ul>
				...
				<p><em>Please Note: Advanced changes to website graphics/colors may require extensive PhotoShop and Web
					...
					<p class="envato-setup-actions step">
						<input type="submit" class="button-primary button button-large button-next"
						value="<?php esc_attr_e( 'Continue' ,'unload'); ?>" name="save_step"/>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							class="button button-large button-next"><?php esc_html_e( 'Skip this step','unload' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
		...
		public function envato_setup_logo_design() {
			...
			?>
			<h1><?php esc_html_e( 'Logo','unload' ); ?></h1>
			<form method="post">
				<p><?php printf( esc_html__( 'Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard. Try %sEnvato Studio%s if you need a new logo designed.' ,'unload'), '<a href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall" target="_blank">', '</a>' ); ?></p>
				...
				<table>
					<tr>
						...
						<td>
							<a href="#" class="button button-upload"><?php esc_html_e( 'Upload New Logo','unload' ); ?></a>
						</td>
					</tr>
				</table>

				<p><em><?php esc_html_e( 'Please Note: Advanced changes to website graphics/colors may require extensive PhotoShop and Web
				Development knowledge. We recommend hiring an expert from','unload' ); ?> <a
				href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall"
				target="_blank">Envato Studio</a> to assist with any advanced website changes.</em></p>
				<div style="display: none;">
					<img src="http://studiotracking.envato.com/aff_i?offer_id=4&aff_id=1564&source=DemoInstall"
		...
		public function envato_setup_logo_design_save() {
			...
			// copied from webinane.theme_options.php
			if ( $new_logo_id ) {
		...
		public function envato_setup_updates() {
			?>
			<h1><?php esc_html_e( 'Theme Updates','unload' ); ?></h1>
			...
			<form method="post">
				...
				if ( count( $my_items ) ) {
					...
					<p class="envato-setup-actions step">
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							class="button button-large button-next button-primary"><?php esc_html_e( 'Continue','unload' ); ?></a>
						</p>
						<?php
					} else {
						?>
						<p><?php esc_html_e( 'Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.','unload' ); ?></p>
						<p><?php esc_html_e( 'When an update becomes available it will show in the Dashboard with an option to install.','unload' ); ?></p>
						<p>
							<em><?php esc_html_e(' On the next page you will be asked to Login with your ThemeForest account and grant
							permissions to enable Automatic Updates. If you have any questions please.','unload' ); ?> <a
							href="https://www.webinane.com/" target="_blank"><?php esc_html_e(' contact us','unload' ); ?></a>.</em>
						</p>
						<p class="envato-setup-actions step">
							<input type="submit" class="button-primary button button-large button-next"
							value="<?php esc_attr_e( 'Login with Envato' ,'unload'); ?>" name="save_step"/>
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
								class="button button-large button-next"><?php esc_html_e( 'Skip this step','unload' ); ?></a>
								<?php wp_nonce_field( 'envato-setup' ); ?>
							</p>
		...
		public function envato_setup_customize() {
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					class="button button-primary button-large button-next"><?php esc_html_e( 'Continue','unload' ); ?></a>
				</p>
			...
			public function envato_setup_help_support() {
				...
				<p>Item Support can be accessed from <a href="https://webinane.com/" target="_blank">https://webinane.com/</a>
				and includes:</p>
				<ul>
				...
				<ul>
					...
					<li>Installation services (this is available through <a
						href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall"
						target="_blank"><?php esc_html_e( 'Envato Studio' ,'unload'); ?> </a>)
					</li>
					<li>Help and Support for non-bundled 3rd party plugins (i.e. plugins you install yourself later on)</li>
				</ul>
				<p><?php esc_html_e( 'More details about item support can be found in the ThemeForest' ,'unload'); ?> <a
					href="http://themeforest.net/page/item_support_policy" target="_blank"><?php esc_html_e( 'Item Support Policy' ,'unload'); ?></a>. </p>
					<p class="envato-setup-actions step">
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							class="button button-primary button-large button-next"><?php esc_html_e( 'Agree and Continue' ,'unload'); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
						<?php
					}
		...
		public function envato_setup_ready() {
			...
			data-url="http://themeforest.net/user/webinane/portfolio?ref=webinane"
			...
			<h1><?php esc_html_e( 'Your Website is Ready!','unload' ); ?></h1>
			...
			<p>Please come back and <a href="http://themeforest.net/downloads" target="_blank">leave a 5-star rating</a>
				if you are happy with this theme. <br/>Follow <a href="https://twitter.com/webinane" target="_blank">@webinane</a>
			on Twitter to see updates. Thanks! </p>

			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					<h2><?php esc_html_e( 'Next Steps','unload' ); ?></h2>
					<ul>
						<li class="setup-product"><a class="button button-primary button-large"
							href="https://twitter.com/@webinane"
							target="_blank"><?php esc_html_e( 'Follow @webinane on Twitter','unload' ); ?></a>
						</li>
						<li class="setup-product"><a class="button button-next button-large"
							href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'View your new website!' ,'unload'); ?></a>
						</li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<h2><?php esc_html_e( 'More Resources','unload' ); ?></h2>
					<ul>
						<li class="documentation"><a href="https://webinane.ticksy.com/"
							target="_blank"><?php esc_html_e( 'Read the Theme Documentation','unload' ); ?></a>
						</li>
						<li class="howto"><a href="https://wordpress.org/support/"
							target="_blank"><?php esc_html_e( 'Learn how to use WordPress','unload' ); ?></a>
						</li>
						<li class="rating"><a href="http://themeforest.net/downloads"
							target="_blank"><?php esc_html_e( 'Leave an Item Rating' ,'unload'); ?></a></li>
							<li class="support"><a href="https://webinane.ticksy.com/"
								target="_blank"><?php esc_html_e( 'Get Help and Support','unload' ); ?></a></li>
							</ul>
						</div>
					</div>
					<?php
				...
				// request the token from our bounce url.
					...
					// the first time this key is used, it is set and locked on the server.
						...
						if ( is_wp_error( $response ) ) {
							...
							$class         = 'error';
							echo "<div class=\"$class\"><p>" . sprintf( esc_html__( 'Something went wrong while trying to retrieve oauth token: %s','unload' ), $error_message ) . '</p></div>';
						} else {
					...
					add_settings_section(
						...
						sprintf( esc_html__( 'Login for %s updates','unload' ), $this->envato_username ),
				...
				if ( !$option || empty($option['oauth']) || empty( $option['oauth'][ $this->envato_username ] ) || empty($envato_items) || empty($envato_items['items']) || !envato_market()->items()->themes( 'purchased' )) {
					...
					if ( ! $dissmissed_time || $dissmissed_time < strtotime('-7 days') ) {
						...
						<div class="notice notice-warning notice-dtbwp-themeupdates is-dismissible">
							<p><?php
							_e( 'Please activate ThemeForest updates to ensure you have the latest version of this theme.','unload' );
							?></p>
							<p>
								<?php printf( __( '<a class="button button-primary" href="%s">Activate Updates</a>','unload' ),  esc_url($this->get_oauth_login_url( admin_url( 'admin.php?page=' . envato_market()->get_slug() . '' ) ) ) ); ?>
							</p>
						</div>
		...
		function put_contents( $file, $content, $permissions = '077' ) {
			global $wp_filesystem;
			...
			$this->file_system->put_contents( $file, $content, $permissions );

		}

		function revslider_export($dir = '') {
			...
			foreach ($sliders as $k => $s) {
				...
				foreach ($slides as $ss) {
					...
					if ($slider_id == unload_set($ss, 'slider_id'))
						$data['slider'][$k]['slides'][] = $ss;
				}
			}


            //$dir = $dir . 'revslider_options';
            //$this->newdir($dir);
			$w_file = $dir . '/' . $file . '.json';
			$this->put_contents($w_file, wp_json_encode( $data ), 0777);
		}
		function revslider_import($file = '') {

			global $wpdb;
            $settings = $this->_get_json( 'revslider.json' );//($file) ? $file : 'default_settings.json';
            //$settings = $this->read_file($this->path . 'revslider_options/' . $file);
        ...
        function layerslider_export($dir = '') {
            ...
            //$slides = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "layerslider", ARRAY_A);
            ...
            function layerslider_import($file = '') {
            	...
            	if ( ! $file ) {
            		$stylefolder = basename(get_theme_mod('dtbwp_site_style',$this->get_default_theme_style()));
            		$theme_style = __DIR__ . '/content/'.$stylefolder.'/';

            		$file = $theme_style . 'layerslider.zip';
            	}
            	...
            	global $wpdb;
            $settings = $this->_get_json( 'layerslider.json' );//($file) ? $file : 'default_settings.json';
            //$settings = $this->read_file($this->path . 'revslider_options/' . $file);
            ...
            if ($settings) {
            	foreach ((array) $settings['slider'] as $v) {
            		...
            		$res = $wpdb->get_results("SELECT * FROM `" . $wpdb->prefix . "layerslider` WHERE `title` LIKE '%" . $v['title'] . "%'");
            		...
            		$wpdb->insert($wpdb->prefix . "layerslider", $v);
            		...
            		if ($slider_id) {
            			...
            			foreach ($slides as $key => $val) {
            				if ($val) {
            					$val['id'] = $slider_id;
            					$wpdb->insert($wpdb->prefix . "layerslider", $val);
            				}
            			}
            		}
            	}
        ...
        function remaining_media($file = '') {
        	global $wp_filesystem;
        	if (empty($wp_filesystem)) {
        		require_once(ABSPATH . '/wp-admin/includes/file.php');
        		WP_Filesystem();
        	}
        	$file = get_template_directory() . '/envato_setup/content/style1/media.xml';
        	if (file_exists($file)) {
